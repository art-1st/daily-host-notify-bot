...
export async function commandRemoveUser(payload: CommandRequestPayload) {
  ...
  try {
    ...
    if (!channel.hasUser(user.id)) {
      throw new CommandError(CommandErrorName.SET_USER_NOT_EXISTS);
    }
      ...
      blocks: [
        {
          ...
          text: {
            type: "mrkdwn",
            // text: `<@${user.id}> (Member ID: ${user.id})님이 삭제되었습니다.`,
            text: `<${user.id}> (Member ID: ${user.id})님이 삭제되었습니다.`,
          },
        },
      ],
